...
> **⚠️ 如果花体设置后过一会自己变回普通字母**：这通常**不是 Telegram 把它过滤掉了**，而是服务器上还有另一个进程/服务在改同一个 Telegram 账号的昵称——常见于装过别的同类脚本、或者重复部署过本项目、残留了旧的 systemd 服务/cron 没清理。排查方法：
> ```bash
> systemctl list-units --type=service --all | grep -Ei 'tg|profile|telegram|clock'
> ps aux | grep -i python
> crontab -l
> ```
> 找到多余的服务/进程后 `systemctl disable --now <服务名>` 停掉，只留 `tg-profile.service`（或你手动跑的那个 `app.py run`）一个在改这个账号即可。本项目会用 `_self_check` 定期自检当前昵称有没有被"外部"改掉并打 warning 日志，方便发现这类冲突，但**判断原因时不要先假设是 Telegram 的问题**，先查是不是自己机器上有别的脚本在抢着改。

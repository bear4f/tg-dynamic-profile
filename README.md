...
> **⚠️ 实测已确认的平台限制**：设置非 `plain` 的样式后，Telegram **一开始会正常接受**花体字符（`account.updateProfile` 调用当时的回包也能确认），但大约 **20~30 秒后**，Telegram 会在后台异步把这些字符悄悄还原成普通字母——这是平台自己的反仿冒扫描，不是本程序的 bug，代码层面也无法绕过。实际效果就是：每次刷新昵称后，前二十几秒是花体、之后变回普通字母，`update_interval` 设得越大，"变回默认"占的时间比例就越高，看起来跟"随机"差不多。如果不想要这种忽变忽不变的效果，直接用默认的 `plain` 即可。这个结论是从运行日志里 `_self_check`（定期主动重新查询 Telegram 现在存的名字）打出的 `自检发现昵称被后台改成了 ...，设置后约 N 秒发生` 这条 warning 实测确认的。
